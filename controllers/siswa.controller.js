...
const SPPModel = require("../models/spp.model");
const { generateCounterID, getMonthName } = require("../utils/db.util");
...
router.post('/', [isAuthenticated], async (req, res) => {
  try {
    ...
    for (nomor in data) {
      if (bulanTahunAjaran[nomor] === 0) { year = year + 1 }
      spp.push({
        nomor: data[nomor],
        jatuhTempo: new Date(year, bulanTahunAjaran[nomor], 11),
...
router.get('/:nis', [isAuthenticated], async (req, res) => {
  try {
    let siswa = await SiswaModel.findOne({nis: req.params.nis}, {_id: 0, __v: 0})
    if (!siswa) {
...
router.put('/:nis', [isAuthenticated], async (req, res) => {
  try {
    let siswa = await SiswaModel.findOne({kode: req.params.siswa}, {_id: 0, __v: 0})
    if (!siswa) {
      return res.status(404).json({ message: 'Siswa not found!' })
    }
    ...
    siswa = await SiswaModel.findOne({nis: req.params.nis}, {_id: 0, __v: 0})
    return res.json(siswa)
  } catch (error) {
    return res.status(400).json(error)
  }
