...
router.get('/', [isAuthenticated], async (req, res) => {
  try {
    // periksa nis siswa
    let siswa = await SiswaModel.findOne({nis: req.query.nis})
    if (!siswa) {
      return res.status(404).json({ message: "Nis tidak terdaftar!" })
    }

    // ambil spp
    let daftarSPP = await SPPModel.find({ siswa: req.query.nis, tahunAjaran: req.query.tahunAjaran }, { _id: 0, __v: 0 })
    if (!daftarSPP) {
...
router.put('/:nomor', async (req, res) => {
  try {
    // ambil spp
    let spp = await SPPModel.findOne({ nomor: req.params.nomor }, { __v: 0, _id: 0 })
    if (!spp) {
    ...
    if (tanggalBayar > spp.tanggalBayar) {
